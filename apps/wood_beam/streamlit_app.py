...
import math, re, os, datetime, tempfile, io

# ───────────────────────────────────────────────────────────────
#  WOOD + PSL + LVL + STEEL DATABASES
# ───────────────────────────────────────────────────────────────
...
wood_list = []
for desc, mat, w, d, plf, fb, e in wood_sections_raw:
    I = w * d**3 / 12
    S = w * d**2 / 6
    wood_list.append((desc, mat, w, d, plf, fb, e, I, S))
...
def max_defl_ss(L_ft, pls, dls, E, I):
    return max(abs(_ss_defl(i*L_ft/1000, L_ft, pls, dls, E, I))
               for i in range(1001))
...
def max_defl_cant(L_ft, pls, dls, E, I):
    return max(abs(_cant_defl(i*L_ft/1000, L_ft, pls, dls, E, I))
               for i in range(1001))

# ───────────────────────────────────────────────────────────────
#  MOMENT + REACTION FUNCTIONS
# ───────────────────────────────────────────────────────────────

def compute_main_span_moment(L, pls, dls):
    RA = sum(P*(L-a)/L for a,P in pls) + \
         sum(w*(x2-x1)*(L-(x1+(x2-x1)/2))/L for x1,x2,w in dls)

    RB = sum(P*a/L for a,P in pls) + \
         sum(w*(x2-x1)*((x1+(x2-x1)/2))/L for x1,x2,w in dls)
    ...
    crit = sorted({0.0, L} |
                  {a for a,_ in pls} |
                  {c for x1,x2,_ in dls for c in (x1,x2)})
...
def compute_cantilever_reaction(L, pls, dls):
    return sum(P for _,P in pls) + \
           sum(w*(x2-x1) for x1,x2,w in dls)

# ───────────────────────────────────────────────────────────────
#  SECTION SELECTION
# ───────────────────────────────────────────────────────────────

def select_wood_beam(M, L, pls, dls, defl_limit,
                     const_dim, const_value, is_cant, mat_filter=None):
    ...
    for desc,mat,w,d,plf,fb,e,Ix,Sx in wood_list:
        ...
        defl = (max_defl_cant(L,pls,dls,e,Ix)
                if is_cant else
                max_defl_ss(L,pls,dls,e,Ix))
    ...
    for beam_id in [1,2,3]:
        ...
        for x1, x2, w in dl:
            ...
            if beam_id == 1:
                ...
                px1, px2 = main_len + x1, main_len + x2

            h = 0.42 * (abs(w) / max_w) + 0.08
            ax.add_patch(patches.Rectangle(
                (px1, 0), px2 - px1, h,
                linewidth=1.5, edgecolor='blue',
                facecolor='lightblue', alpha=0.45
            ))

            xv = float(math.ceil(px1 + 1e-6))
            while xv < px2 - 1e-6:
                ax.plot([xv, xv], [0, h], 'b-', linewidth=0.8, alpha=0.7)
                xv += 1.0
...
# ───────────────────────────────────────────────────────────────
#  LATEX GENERATOR (TEX ONLY, NO PDF)
# ───────────────────────────────────────────────────────────────
...
def generate_latex_content(loc, beam_label, defl_limit, L0, results_summary, max_M):
    ...
    L = [
        ...
        r'\begin{document}',
        r'\begin{center}',
        r'{\LARGE \textbf{BEAM DESIGN REPORT}}\

\[8pt]',
        r'{\large \textbf{Location:} ' + tex_esc(loc) + r'}\

\[4pt]',
        r'{\large \textbf{Beam Type:} ' + tex_esc(beam_label) + r'}\

\[4pt]',
        r'{\large \textbf{Deflection Limit:} L/' + str(defl_limit) + r'}\
...
\[4pt]',
        (f'\\textbf{{Beam Geometry:}}\\quad '
         ...
         f'Max Moment = {max_M:.2f} ft-kips \\\

\[6pt]'),
    ]

    for bid, sname in span_names.items():
        if bid not in results_summary:
            continue
        ...
        L += [
            ...
            r'\begin{tabular}{@{}ll@{}}',
            r'\textbf{Support:} & ' +
            ('Cantilever' if r['is_cant'] else 'Simply Supported') + r' \\',
            r'\textbf{Max Moment:} & ' + f'{r["M"]:.2f} ft-kips' + r' \\',
            r'\textbf{Defl. Limit:} & L/' + str(defl_limit) +
            f' = {da:.3f} in' + r' \\',
        ]
        ...
        if r['type'] == 'steel':
            L += [
                ...
                r'\textbf{Reactions R1/R2:} & ' +
                f'{r["R1"]:.2f} k / {r["R2"]:.2f} k' + r' \\',
            ]

        elif r['type'] == 'wood':
            react = (
                r'\textbf{Reaction (fixed):} & ' +
                f'{r["R1"]:.2f} kips' + r' \\'
                if r['is_cant'] else
                r'\textbf{Reactions R1/R2:} & ' +
                f'{r["R1"]:.2f} k / {r["R2"]:.2f} k' + r' \\'
            ...
            L += [
                ...
                r'\textbf{Selected:} & \textbf{' +
                tex_esc(r['desc']) + r'} \\',
                r'\textbf{Dimensions:} & ' +
                f'{r["width"]:.3f}" × {r["depth"]:.1f}"' + r' \\',
                r'\textbf{Self Weight:} & ' +
                f'{r["plf"]:.1f} lb/ft' + r' \\',
                r'\textbf{Req. S:} & ' +
                f'{r["S_req"]:.1f} in$^3$' + r' \\',
                r'\textbf{Prov. I:} & ' +
                f'{r["I_prov"]:.0f} in$^4$' + r' \\',
                r'\textbf{Prov. S:} & ' +
                f'{r["S_prov"]:.1f} in$^3$' + r' \\',
                r'\textbf{Deflection:} & ' +
                f'{r["defl"]:.3f} in' + r' \\',
                react,
            ]
            ...
            L.append(
                r'\multicolumn{2}{l}{\textcolor{red}{No adequate section found.}} \\'
            )
...
def main():
    ...
    st.title("🏗 Wood / PSL / LVL / Steel Beam Design Calculator")
    st.caption("Cantilever + Main Span | Sawn Lumber · PSL · LVL · Steel")
    st.divider()
    ...
    if force_lvl:
        mat_filter = "lvl"
        st.info("LVL only: 1.75\" × 14\" / 16\" (1–4 plies)")
    ...
    if not force_steel:
        with st.expander("Dimension Constraints (optional)"):
            ...
            if constr == "Max Depth":
                const_dim = 'D'
                const_value = cc2.number_input("Max depth (in)", 6.0, 36.0, 18.0)
    ...
    if st.button("⚡  Calculate", type="primary", use_container_width=True):
        ...
        for bid, sname, slen, is_cant, pfx in span_cfg:
            ...
            if force_steel:
                ...
                entry.update({
                    'type': 'steel', 'desc': desc,
                    'S_req': Sr, 'I_prov': Ip, 'S_prov': Sp, 'defl': defl
                })

            else:
                wr = select_wood_beam(M, L, pl, dl, defl_limit,
                                      const_dim, const_value, is_cant, mat_filter)
                ...
                if wr is None:
                    ...
                    entry.update({
                        'type': 'wood', 'desc': desc, 'mat': mat,
                        'S_req': Sr, 'I_prov': Ip, 'S_prov': Sp,
                        'defl': defl, 'width': width, 'depth': depth, 'plf': plf
                    })
        ...
        d2.download_button(
            "⬇ LaTeX (.tex)",
            tex_str,
            file_name="beam_report.tex",
            mime="text/plain",
            use_container_width=True
        )

        st.info("PDF generation removed — use Overleaf to compile the .tex file.")
